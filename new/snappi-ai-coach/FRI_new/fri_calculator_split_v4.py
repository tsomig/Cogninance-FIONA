"""
Financial Resilience Index (FRI) Calculator v4 — Snappi Production Engine
==========================================================================
Processes raw Snappi core banking transactions through a simplified
direction-based FRI Category Map (v4) to compute Buffer (45%),
Stability (30%), and Momentum (25%) components.

v4 changes from v3:
  - Classification uses (TransactionType, direction) instead of
    (TransactionType, TransactionSubSubType) — 43 map entries vs 178
  - Direction derived from CreditAmountLC / DebitAmountLC columns
  - savings_balance defaults to current_balance (RunningBalance model)
  - Removed MCC enrichment pass (can be re-added when Paymentology data available)
...
from fri_category_map_v4 import (
    FRI_CATEGORY_MAP,
    DEFAULT_ROLE,
...
class FRIResult:
    ...
    @property
    def interpretation_band(self) -> dict:
        """Full interpretation context including thresholds and distance."""
        bands = [
            (80, 100, 'Thriving',    'Excellent financial resilience'),
            (60,  80, 'Stable',      'Good financial health'),
            (40,  60, 'Vulnerable',  'Needs attention'),
            (20,  40, 'Fragile',     'Requires support'),
            ( 0,  20, 'Crisis',      'Urgent intervention needed'),
        ]
        for low, high, label, desc in bands:
            if self.total_score >= low:
                return {
                    'band': label,
                    'description': desc,
                    'band_range': [low, high],
                    'score': round(self.total_score, 2),
                    'distance_to_next_band': round(high - self.total_score, 2) if high < 100 else None,
                    'distance_from_lower_band': round(self.total_score - low, 2),
                }
        return {'band': 'Crisis', 'description': 'Urgent intervention needed',
                'band_range': [0, 20], 'score': round(self.total_score, 2),
                'distance_to_next_band': round(20 - self.total_score, 2),
                'distance_from_lower_band': round(self.total_score, 2)}

    def to_dict(self) -> dict:
        """
        Exhaustive FRI output — every computed value surfaced.

        Structure (7 top-level blocks):
          1. scores          – headline FRI + per-component with weighted contributions
          2. interpretation  – band, thresholds, distance to next band
          3. configuration   – all parameters, weights rationale, life stage
          4. data_quality    – confidence, provisioning, completeness detail
          5. transaction_metadata – counts, date range, role distribution
          6. components      – full detail dicts for Buffer, Stability, Momentum
          7. coaching_signals – behavioral triggers with approach directives
        """
        w = self.weights
        return {
            # ── 1. SCORES ───────────────────────────────────────
            ...
            'components': {
                'buffer': {
                    'score': round(self.buffer, 2),
                    'weight': w['buffer'],
                    'weighted_contribution': round(self.buffer * w['buffer'], 2),
                },
                'stability': {
                    'score': round(self.stability, 2),
                    'weight': w['stability'],
                    'weighted_contribution': round(self.stability * w['stability'], 2),
                },
                'momentum': {
                    'score': round(self.momentum, 2),
                    'weight': w['momentum'],
                    'weighted_contribution': round(self.momentum * w['momentum'], 2),
                },
            },

            # ── 2. INTERPRETATION ───────────────────────────────
            'interpretation': self.interpretation_band,
            ...
            # ── 7. COMPONENT DETAILS ────────────────────────────
            'buffer_detail': self.buffer_detail,
            'stability_detail': self.stability_detail,
            'momentum_detail': self.momentum_detail,
...
class TransactionClassifier:
    """
    Classifies raw Snappi transactions into FRI roles using direction-based
    lookup: (TransactionType, inflow/outflow/neutral) → FRI role.

    Direction is derived from CreditAmountLC / DebitAmountLC columns.
    """
    ...
    def classify(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['fri_role'] = df.apply(
            lambda row: classify_transaction(
                row.get('TransactionType', ''),
                row.get('CreditAmountLC', 0) or 0,
                row.get('DebitAmountLC', 0) or 0,
            ),
            axis=1,
        )

        # Essential flag: True for spending, fees, taxes, debt service
        essential_roles = ESSENTIAL_SPENDING_ROLES | UNCLASSIFIED_SPENDING_ROLES | DEBT_SERVICE_ROLES | DEBT_COST_ROLES | INCOME_ROLES
        df['fri_essential'] = df['fri_role'].isin(essential_roles)

        # Enrichment flag: True for unclassified spending (MCC would help)
        df['fri_needs_enrichment'] = df['fri_role'] == 'BUFFER_SPENDING_UNCLASSIFIED'
...
class FRICalculator:
    ...
    def calculate(
        ...
        """
        Calculate FRI from transaction data.

        Parameters
        ----------
        current_balance : float
            Last RunningBalance value from the account.
        savings_balance : float, optional
            If None, defaults to current_balance (simplified model where
            savings ≡ current balance from RunningBalance column).
        """
        ...
        coaching_signals = self._detect_coaching_signals(
            classified, buffer, stability, momentum, momentum_detail, calc_date, weights
        )

        # ── Classification summary: role distribution with EURO amounts ──
        role_counts = classified['fri_role'].value_counts().to_dict()
        enrichment_mask = classified['fri_needs_enrichment']

        # Per-role euro breakdown (what the LLM needs to say "€X went to Y")
        role_euro_detail = {}
        for role in sorted(classified['fri_role'].unique()):
            subset = classified[classified['fri_role'] == role]
            credit = float(subset['fri_net_amount'].clip(lower=0).sum())
            debit = float(abs(subset['fri_net_amount'].clip(upper=0).sum()))
            role_euro_detail[role] = {
        ...
        classification_summary = {
            ...
            'role_euro_detail': role_euro_detail,
            'enrichment_needed_count': int(enrichment_mask.sum()),
            'enrichment_needed_pct': round(
                enrichment_mask.sum() / len(classified) * 100, 1
            ) if len(classified) > 0 else 0.0,
            'production_ready_count': int((~enrichment_mask).sum()),
            'production_ready_pct': round(
                (~enrichment_mask).sum() / len(classified) * 100, 1
            ) if len(classified) > 0 else 0.0,
        }

        # ── Financial summary: cash flow, debt lifecycle, fees, rewards ──
        financial_summary = self._build_financial_summary(classified, calc_date)

        # ── Transaction metadata: date range, volume, span ──
        dated = classified[classified['transaction_date'].notna()]
        if not dated.empty:
            earliest = dated['transaction_date'].min()
            latest = dated['transaction_date'].max()
            tx_metadata = {
                ...
                'months_span': months_available,
                'months_with_transactions': int(
                    dated['transaction_date'].dt.to_period('M').nunique()
                ),
                'avg_transactions_per_month': round(
                    len(dated) / max(1, months_available), 1
                ),
            }
        else:
            tx_metadata = {
        ...
        # ── Configuration: all parameters that shaped this result ──
        life_stage = self._get_life_stage(age)
        life_stage_cfg = LIFE_STAGE_CONFIG[life_stage]
        weight_reason = (
            f"data_mode={data_mode}" if data_mode != 'full_data'
            else f"income_segment={income_segment} (stability={stability:.1f})"
        )
        configuration = {
            'data_mode': data_mode,
            ...
            'life_stage_detail': {
                'age_input': age,
                'age_range': list(life_stage_cfg['age_range']),
                'target_buffer_months': life_stage_cfg['target_months'],
                'buffer_scale_factor': life_stage_cfg['scale_factor'],
            },
            'essential_ratio_fallback': self.essential_ratio_fallback,
            'momentum_alpha': momentum_detail.get('alpha', 0.6),
            'momentum_k': momentum_detail.get('k', 2.0),
        }
    ...
    def _calculate_buffer(
        ...
        # Layer 1: Identified essential (direct debit, fees, tax — NOT debt repayment)
        ...
        # Layer 1b: Debt servicing (BNPL/Flex repayments — separate obligation layer)
        debt_svc_mask = recent['fri_role'].isin(DEBT_SERVICE_ROLES)
        identified_debt_service = abs(
            recent.loc[debt_svc_mask & (recent['fri_net_amount'] < 0), 'fri_net_amount'].sum()
        )
        ...
        total_essential_3m = identified_essential + identified_debt_service + mcc_enriched_essential + estimated_essential
        months_in_window = 2.96  # It ensures that the Net Financial Rate and the Buffer score reflect reality without the 1.5% frugality bias introduced by calendar drift, if 3.0 is used.
        # this is a correct divisor for a strict 90-day lookback window
        ...
        detail = {
            ...
            'life_stage': self._get_life_stage(age),
            'target_buffer_months': LIFE_STAGE_CONFIG[self._get_life_stage(age)]['target_months'],
            'avg_monthly_essential': round(avg_monthly_essential, 2),
            'identified_essential_3m': round(identified_essential, 2),
            'identified_debt_service_3m': round(identified_debt_service, 2),
            ...
            'buffer_months_vs_target': round(
                (liquid_assets / avg_monthly_essential)
                - LIFE_STAGE_CONFIG[self._get_life_stage(age)]['target_months'], 2
            ) if avg_monthly_essential > 0 else None,
            'buffer_coverage_assessment': (
                'adequate' if buffer >= 60
                else 'thin' if buffer >= 30
                else 'critically_low'
            ),
            'essential_breakdown_quality': (
                'mcc_enriched' if mcc_enriched_essential > 0
                else 'identified_only' if identified_essential > 0 and total_unclassified == 0
                else 'estimated' if total_unclassified > 0
                else 'no_spending_detected'
            ),
        }
    ...
    def _calculate_stability(
        ...
        # Build period-labeled income series
        monthly_income_labeled = [
            {'month': str(period), 'income': round(float(val), 2)}
            for period, val in zip(monthly_income.index, income_values)
        ]

        return stability, {
            'status': 'computed',
            'monthly_income': [round(v, 2) for v in income_values],
            'monthly_income_labeled': monthly_income_labeled,
            'mean': round(mean_income, 2),
            'std': round(std_income, 2),
            'cv': round(cv, 4),
            'cv_interpretation': (
                f"Income varies by {round(cv * 100, 1)}% around the mean — "
                + ('very predictable' if cv < 0.15
                   else 'mostly predictable' if cv < 0.30
                   else 'moderately variable' if cv < 0.50
                   else 'highly variable' if cv < 0.80
                   else 'extremely variable')
            ),
            'months_with_income': months_with_income,
            'months_analyzed': len(income_values),
            'months_without_income': int(len(income_values) - months_with_income),
            'income_range': {
                'min': round(float(np.min(income_values)), 2),
                'max': round(float(np.max(income_values)), 2),
                'range': round(float(np.max(income_values) - np.min(income_values)), 2),
            },
            'income_regularity_assessment': (
                'very_stable' if cv < 0.15
                else 'stable' if cv < 0.30
                else 'moderate_variability' if cv < 0.50
                else 'high_variability' if cv < 0.80
                else 'extreme_variability'
            ),
            'income_segment_implied': (
                'STABLE_SALARIED' if stability >= 85
                else 'VARIABLE_INCOME' if stability >= 60
    ...
    # ────────────────────────────────────────────────────────────────────
    # FINANCIAL SUMMARY: Cash flow, monthly series, debt lifecycle
    # ────────────────────────────────────────────────────────────────────
    ...
    def _build_financial_summary(
        ...
        """
        Aggregate financial picture from classified transactions.

        Surfaces everything the LLM needs to narrate the user's financial
        story: total cash flow, monthly series, debt lifecycle, fees,
        taxes, rewards, and spending composition.
        """
        dated = df[df['transaction_date'].notna()].copy()

        # ── Lifetime cash flow ──
        total_inflows = float(dated.loc[dated['fri_net_amount'] > 0, 'fri_net_amount'].sum())
        total_outflows = float(abs(dated.loc[dated['fri_net_amount'] < 0, 'fri_net_amount'].sum()))
        ...
        # ── Monthly cash flow series (period-labeled) ──
        monthly_cash_flow = []
        if not dated.empty:
            dated['_period'] = dated['transaction_date'].dt.to_period('M')
            for period in sorted(dated['_period'].unique()):
                month_data = dated[dated['_period'] == period]
                m_in = float(month_data.loc[month_data['fri_net_amount'] > 0, 'fri_net_amount'].sum())
                m_out = float(abs(month_data.loc[month_data['fri_net_amount'] < 0, 'fri_net_amount'].sum()))

                # Per-role breakdown for this month
                role_breakdown = {}
                for role in month_data['fri_role'].unique():
                    r_subset = month_data[month_data['fri_role'] == role]
                    r_credit = float(r_subset['fri_net_amount'].clip(lower=0).sum())
                    r_debit = float(abs(r_subset['fri_net_amount'].clip(upper=0).sum()))
                    role_breakdown[role] = {
                        'credit': round(r_credit, 2),
                        'debit': round(r_debit, 2),
                        'net': round(r_credit - r_debit, 2),
                        'count': int(len(r_subset)),
                    }

                monthly_cash_flow.append({
                    'month': str(period),
                    'inflows': round(m_in, 2),
                    'outflows': round(m_out, 2),
                    'net': round(m_in - m_out, 2),
                    'transaction_count': int(len(month_data)),
                    'role_breakdown': role_breakdown,
                })
            dated.drop(columns=['_period'], inplace=True)

        # ── Debt lifecycle (full history) ──
        total_borrowed = float(dated.loc[
            dated['fri_role'].isin(DEBT_INCREASE_ROLES) & (dated['fri_net_amount'] > 0),
            'fri_net_amount'
        ].sum())
        total_repaid_outflows = float(abs(dated.loc[
            dated['fri_role'].isin(DEBT_DECREASE_ROLES) & (dated['fri_net_amount'] < 0),
            'fri_net_amount'
        ].sum()))
        total_refund_inflows = float(dated.loc[
            dated['fri_role'].isin(DEBT_DECREASE_ROLES) & (dated['fri_net_amount'] > 0),
            'fri_net_amount'
        ].sum())
        total_debt_costs = float(abs(dated.loc[
            dated['fri_role'].isin(DEBT_COST_ROLES) & (dated['fri_net_amount'] < 0),
            'fri_net_amount'
        ].sum()))
        total_repaid = total_repaid_outflows + total_refund_inflows
        outstanding_estimate = total_borrowed - total_repaid

        debt_lifecycle = {
            'total_borrowed': round(total_borrowed, 2),
            'total_repaid': round(total_repaid, 2),
            'total_repaid_outflows': round(total_repaid_outflows, 2),
            'total_refund_inflows': round(total_refund_inflows, 2),
            'total_debt_costs': round(total_debt_costs, 2),
            'outstanding_estimate': round(outstanding_estimate, 2),
            'debt_products_opened': int(dated[dated['fri_role'].isin(DEBT_INCREASE_ROLES)].shape[0]),
            'debt_payments_made': int(dated[dated['fri_role'].isin(DEBT_DECREASE_ROLES)].shape[0]),
            'has_active_debt': outstanding_estimate > 0,
            'cost_of_debt_ratio': round(
                total_debt_costs / total_borrowed * 100, 1
            ) if total_borrowed > 0 else 0.0,
            'repayment_progress_pct': round(
                total_repaid / total_borrowed * 100, 1
            ) if total_borrowed > 0 else None,
        }

        # ── Bank fees (lifetime) ──
        fee_debit = float(abs(dated.loc[
            (dated['fri_role'] == 'FEE_BANK') & (dated['fri_net_amount'] < 0),
            'fri_net_amount'
        ].sum()))
        fees_summary = {
            'total_bank_fees': round(fee_debit, 2),
            'fee_transaction_count': int((dated['fri_role'] == 'FEE_BANK').sum()),
        }

        # ── Taxes & levies (lifetime) ──
        tax_debit = float(abs(dated.loc[
            (dated['fri_role'] == 'TAX_LEVY') & (dated['fri_net_amount'] < 0),
            'fri_net_amount'
        ].sum()))
        taxes_summary = {
            'total_taxes_levies': round(tax_debit, 2),
            'tax_transaction_count': int((dated['fri_role'] == 'TAX_LEVY').sum()),
        }

        # ── Rewards & cashback (lifetime) ──
        reward_credit = float(dated.loc[
            (dated['fri_role'] == 'REWARD_CASHBACK') & (dated['fri_net_amount'] > 0),
            'fri_net_amount'
        ].sum())
        rewards_summary = {
            'total_cashback_received': round(reward_credit, 2),
            'reward_transaction_count': int((dated['fri_role'] == 'REWARD_CASHBACK').sum()),
        }

        # ── Income summary (lifetime, complementing stability_detail) ──
        income_total = float(dated.loc[
            dated['fri_role'].isin(INCOME_ROLES) & (dated['fri_net_amount'] > 0),
            'fri_net_amount'
        ].sum())
        income_count = int(dated[dated['fri_role'].isin(INCOME_ROLES)].shape[0])
        income_summary = {
            'total_income_received': round(income_total, 2),
            'income_transaction_count': income_count,
        }

        # ── Spending summary (lifetime) ──
        spending_roles = {'BUFFER_ESSENTIAL', 'BUFFER_DISCRETIONARY', 'BUFFER_SPENDING_UNCLASSIFIED'}
        spending_debit = float(abs(dated.loc[
            dated['fri_role'].isin(spending_roles) & (dated['fri_net_amount'] < 0),
            'fri_net_amount'
        ].sum()))
        spending_summary = {
            'total_spending': round(spending_debit, 2),
            'spending_transaction_count': int(
                dated[dated['fri_role'].isin(spending_roles)].shape[0]
            ),
        }

        # ── Internal transfers & system operations (excluded from FRI) ──
        excluded_credit = float(dated.loc[
            dated['fri_role'].isin(EXCLUDED_ROLES) & (dated['fri_net_amount'] > 0),
            'fri_net_amount'
        ].sum())
        excluded_debit = float(abs(dated.loc[
            dated['fri_role'].isin(EXCLUDED_ROLES) & (dated['fri_net_amount'] < 0),
            'fri_net_amount'
        ].sum()))
        excluded_summary = {
            'total_excluded_credit': round(excluded_credit, 2),
            'total_excluded_debit': round(excluded_debit, 2),
            'excluded_transaction_count': int(
                dated[dated['fri_role'].isin(EXCLUDED_ROLES)].shape[0]
            ),
        }

        return {
            'cash_flow_lifetime': cash_flow_lifetime,
            'monthly_cash_flow': monthly_cash_flow,
            'income_summary': income_summary,
            'spending_summary': spending_summary,
            'debt_lifecycle': debt_lifecycle,
    ...
    def _detect_coaching_signals(
        ...
        fri_approx = weights['buffer'] * buffer + weights['stability'] * stability + weights['momentum'] * momentum
        if fri_approx < 30:
            signals.append({
                'type': 'CRITICAL',
